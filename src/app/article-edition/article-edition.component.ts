...
// Core dependencies for this component
import { NewsService } from '../services/news.service';
import { Article } from '../model/article.model'; // Make sure this path is correct
...
export class ArticleEditionComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    saveOperation.subscribe({
      next: () => {
        this.feedback = { type: 'success', message: `Article successfully ${this.isEditMode ? 'updated' : 'created'}!` };
        setTimeout(() => this.router.navigate(['/']), 2000); // Navigate home after success
      },
      error: (err) => {
        console.error('Failed to save article', err);
        this.feedback = { type: 'error', message: `Failed to ${this.isEditMode ? 'update' : 'create'} article.` };
      }
    });
  }
